...
class Bullet(pygame.sprite.Sprite):
	def __init__(self, image, x, y, angle):
		...
		self.speed = 10
		#calculate the horiz and vert speeds based on angle
...
while run:
	...
	if game_over == False:
		...
		if repair_button.draw(screen):
			if pygame.time.get_ticks() - update_time > BUTTON_COOLDOWN:
				update_time = pygame.time.get_ticks()
				castle.repair()
		...
		#create enemies
		#check if max number of enemies have been reach
		if level_difficulty < target_difficulty:
			if pygame.time.get_ticks() - last_enemy > ENEMY_TIMER:
				#create enemies
				e = random.randint(0, len(enemy_types) - 1)
				yval = random.randint(90, 150)
				enemy_1 = Enemy(enemy_health[e], enemy_animations[e], -100, SCREEN_HEIGHT - yval, 1)
		...
		if next_level == True:
			draw_text('LEVEL COMPLETE!', font_60, GREY, 200, 200)
			#update high score
		...
		draw_text('GAME OVER!', font, RED, 300, 300)
